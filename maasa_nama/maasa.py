...
# %%
def _plot_fm_hist_by_naks(fm_pvt, ax=None, markersize=10) -> None:
	"""
	...
	plt.show()
# %%
def plot_full_moon_hist_by_naks( from_year=-1999 , num_years=2000, chunks=4) -> None:
	...
	fm_df = fm_df[ (fm_df.jd > jd_from_year) & (fm_df.jd < (jd_from_year + num_years*365.25)) ]
	...
	for step in range (1,chunks+1)  :
		# fig,  axs = plt.subplots(4,1, figsize=(16,6), sharex=True)
		# axs = axs.flatten()
...
plot_full_moon_hist_by_naks(-2000, 2000, chunks=600)	
# %%
def plot_full_moon_distance_hist_by_naks() -> None:
	fm_df = get_full_moon_planet_pos(force=not True)
	# pd.options.display.float_format = '{:.2f}'.format
	# display(fm_df.sort_values(by='jd'))
	...
	for maasa in [m for m in [
		'M01-Ash', 'N02-Bha', 'M03-Kri', 'N04-Roh', 'M05-Mrg', 'N06-Ard', 'N07-Pun', 'M08-Pus', 'N09-Asl',
 		'M10-Mag', 'N11-PPal', 'M12-UPal', 'N13-Has', 'M14-Chi', 'N15-Swa', 'M16-Vis', 'N17-Anu', 'M18-Jye',
 		'N19-Mul', 'M20-PAsh', 'N21-UAsh', 'M22-Shr', 'N23-Dha', 'N24-Sha', 'N25-PBha', 'M26-UBha',
		'N27-Rev'
	] if m[0] == 'M'] :
		maasa = int(maasa[1:3])-1
		maasas.append(maasa)

	fm_pvt_elong = fm_df.pivot_table(index=['date', 'jd'], columns='planet', values='elong', aggfunc=np.mean).reset_index()
	fm_pvt_dist = fm_df.pivot_table(index=['date', 'jd'], columns='planet', values='r', aggfunc=np.mean).reset_index()

	xs = pd.cut( (fm_pvt_elong.Moon % 360), [0] + [ x*360/27 for x in range(1,27+1)])
	ys = pd.cut(fm_pvt_dist.Moon, 10)

	xmids = [ v.mid  for v in xs ]
	ymids = [ v.mid  for v in ys ]
	df  = pd.DataFrame( {'x': xmids, 'y':ymids})
	pvt = pd.pivot_table(df, index='x', columns='y', aggfunc=len)
	ax = pvt.iloc[:,[0]].plot(figsize=(16,6), grid=True, marker='*', markersize=10, ls=':', lw=1.5)
	ax.set_xticks(np.arange(0,360,13.34))
	nu = NaksUtils.NaksUtils()
	ax.set_xticklabels(nu.df.nid, rotation=60, fontsize=12)
	for maasa in maasas:
		ax.get_xticklabels()[maasa].set_color("red")
...
if __name__ == "__main__":
	# plot_full_moon_hist_by_naks()
	# plot_moon_gruha_long_diff_hist()

	plot_full_moon_hist_by_naks("r")
...
fm_df = get_full_moon_planet_pos(force=not True)
fm_pvt_dist = fm_df.pivot_table(index=['date', 'jd'], columns='planet', values='r', aggfunc=np.mean).reset_index()
_plot_fm_hist_by_naks(fm_pvt_dist, 15)
#%%
fig, axes = plt.subplots(2,3,figsize=(16,10), sharex=True, sharey=not True)
GRS = ['Saturn', 'Jupiter', 'Mars', 'Venus', 'Mercury', 'Sun']
for ix, p, ax in zip( range(len(GRS)),GRS , axes.flatten()) :
