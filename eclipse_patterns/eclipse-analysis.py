"""
# Converts/cooks the scraped raw NASA eclipse listings to csv form 
#=========================================================================
# ../datasets/moon-phases-scrape-raw.txt => ../datasets/moon-phases-scrape-cooked.csv
#                                              dt,stel_dt,phase,astro_jd,stel_jd,gr
#
# Distribution of eclipse frequency
#=========================================================================
# Plots solar and lunar eclipse frequency in Jaipur region
# The eclipse data is scraped from NASA eclipse javascript page
#
# Other stuff 
#=========================================================================
#  Trying to Fit Astropy JD to Stellarium JD - kinda flop - need to clean up this code
"""
...
from ..maasa_nama.JdUtils import stelAltJD
# %%
...
def j01(x,debugL=None) :
	...
	x = re.sub("^\-","",x)
	try :
		y,m,d,hh,mm =  [ int(z) for z in re.split(r'[\s\-\:]+',x)]
...
#%%
def get_cooked_moon_phases_df(raw_fn = "../datasets/moon-phases-scrape-raw.txt"):
	cooked_csv = re.sub("raw.txt","cooked.csv",raw_fn)
	try:
		1/0
	...
	start_time = time()
	lines_ = lines[:]
	# lines_ = lines[1222:1235+2]
	# lines_ = lines[1235:1235+1]
	nl_ = len(lines_)	
	# print(nl_)
	# print ("".join(lines_))
	for l,nl in zip(lines_, range(nl_)):
		...
		(y,amavas,sgr,s8,poorni,cgr,k8) = ( z.strip() for z in matches) 
		if y: yr = y 
...
cooked_moon_phases_df = get_cooked_moon_phases_df()
cooked_moon_phases_df
#%%
